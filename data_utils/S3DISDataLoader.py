...
class S3DISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        current_points = np.zeros((self.num_point, 9 + 3))  # num_point * (9 + 3)
        # 单轴归一化
        current_points[:, 6] = selected_points[:, 0] / self.room_coord_max[room_idx][0]
        current_points[:, 7] = selected_points[:, 1] / self.room_coord_max[room_idx][1]
        current_points[:, 8] = selected_points[:, 2] / self.room_coord_max[room_idx][2]
        # 三轴归一化
        current_points[:, 9] = selected_points[:, 0] / self.room_coord_max_xyz[room_idx]
        current_points[:, 10] = selected_points[:, 1] / self.room_coord_max_xyz[room_idx]
        current_points[:, 11] = selected_points[:, 2] / self.room_coord_max_xyz[room_idx]
        ...
        return current_points, current_labels, self.room_coord_max_xyz[room_idx]
